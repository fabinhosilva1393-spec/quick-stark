...
// Metallic gradient used by every platform icon.
function MetallicDefs({ id }: { id: string }) {
  return (
    <defs>
      <linearGradient id={id} x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor="#FFFFFF" />
        <stop offset="35%" stopColor="#DCDCE5" />
        <stop offset="70%" stopColor="#9A9AA6" />
...
export function PlatformIcon({ os, className }: Props) {
  return (
    <div className={`platform-icon-wrap ${className || ""}`} aria-hidden="true">
      <div className="platform-icon-glow" />
      {os === "macos" && <MacIcon />}
